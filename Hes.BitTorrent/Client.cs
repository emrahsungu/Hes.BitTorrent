namespace Hes.BitTorrent {
    ...
    public class Client {
        ...
        public void Start() {
            ConsoleLogger.WriteLn("starting client");
        ...
        public void Stop() {
            ConsoleLogger.WriteLn("stopping client");
        ...
        private void HandlePeerListUpdated(object sender, List<IPEndPoint> endPoints) {
            ...
            ConsoleLogger.WriteLn("received peer information from " + (Tracker) sender);
            ConsoleLogger.WriteLn("peer count: " + Peers.Count);
        }
        ...
        private void EnablePeerConnections() {
            ...
            ConsoleLogger.WriteLn("started listening for incoming peer connections on port " + Port);
        }
        ...
        private void DisablePeerConnections() {
            ...
            ConsoleLogger.WriteLn("stopped listening for incoming peer connections on port " + Port);
        }
        ...
        private void HandleBlockCancelled(object sender, DataRequest block) {
            foreach(var item in _outgoingBlocks) {
                if(item.Peer != block.Peer || item.Piece != block.Piece || item.Begin != block.Begin || item.Length != block.Length) continue;
        ...
        private void ProcessUploads() {
            ...
            DataRequest block;
            while(!_uploadThrottle.IsThrottled && _outgoingBlocks.TryDequeue(out block)) {
        ...
        private void ProcessDownloads() {
            ...
            while(_incomingBlocks.TryDequeue(out incomingBlock))
        ...
        private double GetPieceScore(int piece) {
            ...
            var rand = _random.Next(0, 100) / 1000.0;
